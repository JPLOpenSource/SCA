...
/**
 * Visits and extracts all the states from a starting point down.
 * The second, orthogonal-region parameter in constructor
 * {@link #StateVisitor(boolean, AbstractVisitor.OrthoRegion)}
 ...
 */
public class StateVisitor extends AbstractVisitor<State> {
    private static final long serialVersionUID = 9152248607324463429L;
     ...
     */
    public StateVisitor (boolean descend) {
        super(descend);
    }
     ...
     */
    public StateVisitor (boolean descend, OrthoRegion include) {
        super(descend, include);
    }

    @Override
    public void visit (State state) {
        if (!contains(state)) {  // don't duplicate state instances
            add(state);
        }
    }
}
