 ...
 * @see DiagramElement
 */
public class PromelaCHelperStateMachineWriter
{
   ...
   private boolean               executionTraceOn = false;
    ...
    */
   PromelaCHelperStateMachineWriter(StateMachineXmiReader reader) throws Exception
   {
      this.reader = reader;
   ...
   void writeDeclarations() throws Exception
   {
      ...
      declarations.println("#include \"model_sizes.h\""); 
      declarations.println("#include \"model_support.h\""); 
      /* declarations.println("#include \"qhsm.h\"");      */
      declarations.println("");
      // Forward declaration of impl class:
      /* declarations.println("#include \"" + classname + "_impl.h\""); */
      declarations.println("");
      ...
      while (newstateIter.hasNext())
      {
         ...
         if (state instanceof CompositeState)
         {
            ...
            {
               writeOneRegionDeclaration((CompositeStateRegion) s);
            }
         }
      }
      // Write QActive class declaration:
      declarations.println("SUBCLASS(" + classname + " , PrmlInter)");
      declarations.println("");
      indent = "  ";
...
*/      
         ...
         throws Exception
   {
      ...
      if (!(state instanceof ConcurrentCompositeState))
      {
         if (state instanceof CompositeState)
         {
            ...
            for ( State childState : ((CompositeState) state).getChildren())
            {
   ...
   void writeDefinitions() throws Exception
   {
      ...
      definitions.println("#include \"" + classname + "_actions.h\"");      
      definitions.println("DEFINE_THIS_FILE;");
      definitions.println();      
      // constructor
      ...
      definitions.println(indent
            + "   QHsmCtor_(&me->super_.super_, (QPseudoState)" + classname
            + "_initial);");
...
*/
         definitions.println(indent + "REPORT_ACTION(" + machine.name().toUpperCase() + "_" + state.name().toUpperCase() + "_ENTRY);" );
      }
/*      if (((SimpleState) state).hasTimeout())
      {
         definitions.println(indent + "timer.startTimer();");
      }
*/      
      if (state instanceof ConcurrentCompositeState)
      {
...
/**               if (null != t.signalName())
               ...
               definitions.println(indent + "if (timer.timedOut()) {" + "");
               indent = "        ";
               writeActionList(t.getActions());
               State targetstate = machine.getTargetState(t.id());
               if (targetstate instanceof DeepHistoryState)
...
**/               
      ...
      if (null != outgoings)
      {
         ...
         while (out.hasNext())
         {
            ...
            if (null != t.signalName() || t.timeout() != null)
            {
               writeSwitchCase(state, t, classname);
            }
         }
      }
...
*/
            definitions.println(indent + "REPORT_ACTION(SEND_" + action.name() + ");");
      } else if (action instanceof CallAction)
      {
         String tempname = action.name().replaceAll("\\)", "").replaceAll("\\(", "");
         // Garth asked that all call actions not be reported, comment in the next 2 lines to put back in
/****         definitions.println(indent + "REPORT_ACTION(" + machine.name().toUpperCase() + "_" 
        		 + tempname + ");");
***/             
//         definitions.println(indent + classname + "Impl_" + tempname
//               + "me->impl)" + ";");
         // //////////////
         // //System.out.println( "int " + classname + "Impl_" + tempname +
         // classname + "Impl * impl, ...)\n{\n}\n");
      }
   }
   ...
   {
      definitions.println(indent + "if( checkGuard(me, "+ junctioncount + ") ) {");
      junctioncount++;
      // //
   ...
   void writeSwitchCase(String signalname, Transition transition, String classname)
      throws Exception
   {
      indent = "    ";
...
//         definitions.println(indent + "LogEvent_log(stateName);");
   ...
   void writeSwitchCase(State state, Transition transition, String classname)
         throws Exception
   {

      if(transition.timeout() != null)
      {
         writeSwitchCase(machine.name().toUpperCase() + "_"+ state.name().toUpperCase() + "_TIMEOUT", transition, classname);
      }
      else
      {
         for (String signalname : transition.getSignalNames())
         {
            writeSwitchCase(signalname, transition, classname);
         }
      }
   }
   ...
   public static void main(String[] args)
   {
      try
      {
         ...
         // XML files:
         new PromelaCHelperStateMachineWriter(reader).writeAllStateMachines();
