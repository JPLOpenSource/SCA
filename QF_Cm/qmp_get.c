...
void *QMPool_get(QMPool *me) {
    ...
    QFreeBlock *fb;
    QF_INT_LOCK_();
    if (me->nFree__ > (QMPoolCtr)0) {              /* free block available? */
        fb = (QFreeBlock *)me->free__;                /* get the free block */
        me->free__ = fb->next;   /* adjust list head to the next free block */
        if (--me->nFree__ < me->nMin__) {                 /* one less block */
    ...
    QS_BEGIN_NOLOCK_(QS_QF_MPOOL_GET, QS_mpObj_, me->start__);
        QS_TIME_();                                            /* timestamp */
        QS_OBJ_(me->start__);            /* the memory managed by this pool */
        QS_MPC_(me->nFree__);      /* the number of free blocks in the pool */
        QS_MPC_(me->nMin__);  /* min number of free blocks ever in the pool */
    QS_END_NOLOCK_();
...
uint32_t QF_getPoolMargin(uint8_t poolId) {
    ...
    QF_INT_LOCK_();
    margin = (uint32_t)QF_pool_[poolId - 1].nMin__;
    QF_INT_UNLOCK_();
