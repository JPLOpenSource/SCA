...
void KeyboardImpl::clearAll () {
	QF::publish(Q_NEW(My::KeyEvent, My::Calc::Clear));
}

void KeyboardImpl::clearEntry () {
	QF::publish(Q_NEW(My::KeyEvent, My::Calc::ClearEntry));
}

void KeyboardImpl::powerOff () {
	QF::publish(Q_NEW(My::KeyEvent, My::Calc::PowerOff));
}

void KeyboardImpl::sendEquals () {
	My::KeyEvent* ke = Q_NEW(My::KeyEvent, My::Calc::Equals);
	ke->keyId = '=';
	QF::publish(ke);
}
...
void KeyboardImpl::sendKey (char arg1) {
	My::KeyEvent* ke;
	if (arg1 == '0') {
		ke = Q_NEW(My::KeyEvent, My::Calc::Digit_0);
	} else {
		ke = Q_NEW(My::KeyEvent, My::Calc::Digit_1_9);
	}
	ke->keyId = arg1;
	QF::publish(ke);
}
...
void KeyboardImpl::sendPoint () {
	My::KeyEvent* ke = Q_NEW(My::KeyEvent, My::Calc::Point);
	ke->keyId = '.';
	QF::publish(ke);
}

void KeyboardImpl::sendOperator (char arg1) {
	My::KeyEvent* ke = Q_NEW(My::KeyEvent, My::Calc::Oper);
	ke->keyId = arg1;
	QF::publish(ke);
}
