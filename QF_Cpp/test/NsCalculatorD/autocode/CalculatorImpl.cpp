...
bool CalculatorImpl::isKeyId (QEvent const* e, char arg1) {
	KeyEvent const* ke = static_cast<KeyEvent const*>(e);
...
bool CalculatorImpl::onError () {
	if (AttributeMapper::get(this, "TESTING")) {  // for unit-testing
		...
		if (rv) {
			...
			ResultEvent* re = Q_NEW(ResultEvent, ReportResult);
...
void CalculatorImpl::compute (QEvent const* e) {
	...
	this->lOperand = 0.0;
	this->rOperand = 0.0;
	if ((static_cast<KeyEvent const*>(e))->keyId != '=') {  // use result in next operation
		this->op(e);
	}
}
