...
//===========================================================================
//
//       File: TestCalculationsImpl.cpp
// Created on: 30-Jul-2011 10:20:20
...
bool TestCalculationsImpl::isInOperand2 () {
	bool rv = (this->calcActive->getCurrentState() == CALCULATOR_ON_OPERAND2);
...
void TestCalculationsImpl::initTest () {
	this->calcActive = static_cast<Calculator*>(QF::active_[TestCalculationsImpl::PRIO_CALC]);
...
void TestCalculationsImpl::sendNextChar () {
	...
	switch (c) {
	case '+':
		QF::publish(Q_NEW(KeyEvent, B_OP_PLUS));
		break;
	case '-':
		QF::publish(Q_NEW(KeyEvent, B_OP_MINUS));
		break;
	case '*':
		QF::publish(Q_NEW(KeyEvent, B_OP_TIMES));
		break;
	case '/':
		QF::publish(Q_NEW(KeyEvent, B_OP_DIVIDE));
		break;
	case '=':
		QF::publish(Q_NEW(KeyEvent, B_EQUAL));
		break;
	case '.':
		QF::publish(Q_NEW(KeyEvent, B_DOT));
		break;
	case '0':
		QF::publish(Q_NEW(KeyEvent, B_0));
		break;
	case '1':
		QF::publish(Q_NEW(KeyEvent, B_1));
		break;
	case '2':
		QF::publish(Q_NEW(KeyEvent, B_2));
		break;
	case '3':
		QF::publish(Q_NEW(KeyEvent, B_3));
		break;
	case '4':
		QF::publish(Q_NEW(KeyEvent, B_4));
		break;
	case '5':
		QF::publish(Q_NEW(KeyEvent, B_5));
		break;
	case '6':
		QF::publish(Q_NEW(KeyEvent, B_6));
		break;
	case '7':
		QF::publish(Q_NEW(KeyEvent, B_7));
		break;
	case '8':
		QF::publish(Q_NEW(KeyEvent, B_8));
		break;
	case '9':
		QF::publish(Q_NEW(KeyEvent, B_9));
		break;
	default:
...
void TestCalculationsImpl::checkResult (QEvent const* e) {
	ResultEvent const* re = static_cast<ResultEvent const*>(e);
	char expStr[256];
