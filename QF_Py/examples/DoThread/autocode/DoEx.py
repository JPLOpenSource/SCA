...
Automatically generated DoEx state machine trace GUI.
Date Created:  07-Oct-2009 16:07:24
Created By:    reder

GUI widget to trace the execution of state machine "DoEx".
"""
from Tkinter import *
...
MAGICDRAW_WINDOW_WIDTH = 1507.0
MAGICDRAW_WINDOW_HEIGHT = 1201.0
...
class DoEx(object):
    ...
    """
    def __init__(self, name, wparam=1507.0, hparam=1201.0, parent=None, big_name=None, size=16):
        """
        ...
        self.s1 = self.canvas.create_rectangle(308.0, 203.0, 448.0, 273.0, fill="#FFFFCC", width=2, outline="blue", tag="s1")
        self.s1Text = self.canvas.create_text(378.0, 208.0, text="s1", anchor=N, font=("Times", 12, "bold"))
        self.s2 = self.canvas.create_rectangle(301.0, 350.0, 462.0, 427.0, fill="#FFFFCC", width=2, outline="blue", tag="s2")
        self.s2Text = self.canvas.create_text(381.0, 355.0, text="s2", anchor=N, font=("Times", 12, "bold"))
        self.Pseudo0 = self.canvas.create_oval(301.0, 133.0, 319.0, 151.0, fill="#0000FF", tag="Pseudo0")

        # Separator elements:

        # Transition diagram elements:
        self.tran0 = self.canvas.create_line(347.0, 350.0, 347.0, 273.0, width=2, arrow=FIRST, fill="red")
        self.tran1 = self.canvas.create_line(431.0, 273.0, 431.0, 350.0, width=2, arrow=FIRST, fill="red")
        self.tran2 = self.canvas.create_line(368.0, 203.0, 368.0, 142.0, 319.0, 142.0, width=2, arrow=FIRST, fill="red")

        # Text diagram elements:
        self.textBox0 = self.canvas.create_text(350.0, 295.0, text="Ev1", anchor=NW, font=("Times", 11));
        self.textBox1 = self.canvas.create_text(434.0, 295.0, text="Ev1", anchor=NW, font=("Times", 11));

        # Map state names to states:
        self.stateDict = {
                          'DoExs1' : self.s1,
                          'DoExs2' : self.s2,
                          'DoExPseudo0' : self.Pseudo0
                          }
    ...
    def ExitState(self, state):
        ...
        Deactivate on exit
        """
        self.canvas.itemconfigure(self.stateDict[state], width=2, outline= "blue")
    ...
    def EnterState(self, state):
        ...
        Activate on entry
        """
        self.canvas.itemconfigure(self.stateDict[state], width=4, outline = "green")
        self.moveActiveState(self.stateTagDict[state])
...
if __name__ == "__main__":
    c = DoEx("DoEx Test",big_name=True)
